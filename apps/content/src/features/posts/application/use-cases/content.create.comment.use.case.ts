import {
  ...
  PhotoUploadStatus,
  Post,
} from '../../../../../prisma/generated/content-client';
...
export class ContentCreateCommentUseCase
  implements ICommandHandler<ContentCreateCommentCommand>
{
  ...
  async execute(command: ContentCreateCommentCommand): Promise<Comment> {
    const newComment = await this.postsPrismaRepository.createComment(
      command.userId,
      command.postId,
      command.message,
    );
