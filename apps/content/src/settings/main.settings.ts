...
const getEnv = (app: INestApplication) => {
  ...
  const port = configService.get<number>('PORT') || configService.get<number>('CONTENT_LOCAL_PORT');
  const rabbit = configService.get<string>('RABBIT_URLS')?.toString() || '';
  const grpc_url = configService.get<string>('CONTENT_GRPC_URL')?.toString() || '';
...
const checkEnv = (envMode: string) => {
  return envMode !== 'DEVELOPMENT' ? 'PORT' : 'CONTENT_LOCAL_PORT'
}
