...
export class UpdatePostStatusOnFileUploadUseCases
  implements ICommandHandler<UpdatePostStatusOnFileUploadCommand>
{
  ...
  async execute(command: UpdatePostStatusOnFileUploadCommand) {
    ...
    await this.postsPrismaRepository.updateStatusForPost(
      command.postId,
      status,
    );
    await this.postsPrismaRepository.createUrlForPost(
      command.postId,
      command.files,
    );
  }
