  ...
  async execute(command: WebHookPaymentCommand) {
    ...
    switch (event.type) {
      case 'customer.subscription.created':
        try {
          const { id: subscriptionId, customer } = event.data.object;
          ...
          const payment = {
            ...
            amount: Object.values(products).find((p) => p.price === priceId)
              ?.amount,
          };
      ...
      case 'invoice.payment_succeeded':
        try {
          ...
          this.delayRabbitService.publishWithDelay(
            'payments_notify_queue',
            sub,
            15000,
          );

          this.rabbitClient.emit('update_profile_account', [
            { userId: sub.userId, paymentAccount: true },
          ]);
        } catch {}
