...
export class PaymentsCronService {
  ...
  constructor(
    ...
    @Inject('RABBITMQ_PAYMENTS_SERVICE')
  ...
  async handleCron() {
    ...
    const {
      active,
      expired,
    }: { active: Payment[]; expired: (Payment | undefined)[] } =
      await this.paymentsRepository.checkExpiers();
    let removeSub = expired.reduce(
      (acc: string[], rem: Payment | undefined): string[] => {
