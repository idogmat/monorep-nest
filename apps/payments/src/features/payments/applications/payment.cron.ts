...
export class PaymentsCronService {
  ...
  async handleCron() {
    console.log('cron')
    key++
    // this.rabbitClient.emit('payments_queue', { message: 'test_payments + ' + key })
    this.delayRabbitService.publishWith30SecondsDelay('delay_payments_queue', { message: 'test_payments + ' + key })
