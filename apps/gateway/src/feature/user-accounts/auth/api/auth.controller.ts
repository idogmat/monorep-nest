...
import {
  AuthMeOutputMapper,
  AuthMeOutputModel,
} from './models/output/auth-me.model';
...
export class AuthController {
  ...
  @Post('login')
  async login(@Req() req, @Res() res, @Body() loginModel: LoginModel) {
  ...
  async verifyEmail(@Query() query: VerifyEmailToken) {
    const result = await this.commandBus.execute(new VerifyEmailCommand(query));
    if (result.hasError()) {
      new ErrorProcessor(result).handleError();
    }
  }
  ...
  async recoveryPassword(@Body() recovery: EmailRecovery) {
    ...
    const result = await this.authService.sendRecoveryCode(
      email,
      recaptchaToken,
    );
    if (result?.hasError?.()) {
