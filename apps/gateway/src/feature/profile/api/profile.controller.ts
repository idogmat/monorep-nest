...
export class ProfileController {
  ...
  async deleteProfilePhotoGrpc(@Req() req: Request) {
    try {
      const userId = req.user?.userId || '';
      console.log(userId);
      const result = await this.profileClientService.deleteProfilePhoto({
        userId,
      });
      console.log(result);
      return result;
  ...
  async getProfilesGrpc(
    ...
    try {
      ...
      const { items, pageNumber, pageSize, totalCount } =
        await this.profileClientService.getProfiles({ userId, query });
  ...
  async updateProfile(
    ...
    const response = await this.profileService.updateProfileData(
      profile as Partial<UpdateUserProfileRequest>,
      req.user.userId,
    );
  ...
  @ApiBearerAuth()
  @UseGuards(AuthGuard)
  async subscribe(@Req() req, @Param('id') id: string) {
