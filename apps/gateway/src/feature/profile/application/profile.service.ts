...
export class ProfileService {
  ...
  async updateProfileData(
    ...
    const result =
      await this.profileClientService.updateProfileData(requestData);

    return result;
  }
  async subscribe(userId, subscribeUserId) {
    try {
      console.log(userId, subscribeUserId);
      const response = await this.profileClientService.subscribeProfile(
        userId,
        subscribeUserId,
      );
  ...
  async subscribeProfile(userId: string, paymentAccount: boolean) {
    try {
      const response =
        await this.profileClientService.updateUserProfileSubscribe(
          userId,
          paymentAccount,
        );
      return response;
