...
import {
  PaginationSearchFollowersGqlTerm,
  PaginationSearchPaymentGqlTerm,
  PaginationSearchUserGqlTerm,
...
export class SuperAdminService {
  constructor(
    ...
    @Inject('RABBITMQ_POST_BAN_SERVICE')
    private readonly rabbitClient: ClientProxy,
  ) {}
  async findUsers(
    query: PaginationSearchUserGqlTerm,
  ): Promise<{ users: any[]; totalCount: number }> {
    try {
      const { users, totalCount } =
        await this.usersService.getAllUsersGql(query);
      const ids = users.map((u) => u.id);
      const { profiles } = await this.profileClientService.getUserProfilesGQL({
        users: ids,
      });
      const mappedProfiles = profiles.map(
        this.profileMappingService.profileMapping,
      );
      return {
  ...
  async findPayments(
    query: PaginationSearchPaymentGqlTerm,
  ): Promise<{ items: any[]; totalCount: number }> {
    try {
      console.log(query);
      const { items, totalCount } =
        await this.paymentsClientService.getSubscribesGql(query);
  ...
  async getFollowers(
    query: PaginationSearchFollowersGqlTerm,
  ): Promise<{ items: any[]; totalCount: number }> {
    try {
      console.log(query);
      const { items, totalCount } =
        await this.profileClientService.getFollowersGql(query);
  ...
  async getFollowing(
    ...
    try {
      console.log(query);
      const { items, totalCount } =
        await this.profileClientService.getFollowingGql(query);
      console.log({ items, totalCount }, 'gate');
      return { items, totalCount };
    } catch (e) {
      console.log(e, 'fail');
  ...
  async deleteUser(userId: string): Promise<boolean> {
    try {
      const profileUserDeleted =
        await this.profileClientService.deleteProfilesGQL({ userId });
      console.log(profileUserDeleted);
      const sessionsUserDeleted =
        await this.deviceService.deleteAllSessionBeforDeleteUser(userId);
  ...
  async banUser(userId: string, bannedReason: string): Promise<boolean> {
    try {
      const profileUserBanned = await this.profileClientService.banProfileGQL({
        userId,
      });
