...
export class AppController {
  ...
  async GetUserProfiles(data: UserProfileQueryRequest) {
    ...
    const { items, pageNumber, pageSize, totalCount } =
      await this.profileService.findMany(data.query);
    // console.log(result)
    const mapped = items.map((p) =>
      OutputProfileModelMapper(p, profileForMatchId),
    );
  ...
  async updateProfileGrpc(data: UpdateUserProfileRequest) {
  ...
  async updateProfileDataGrpc(
    ...
    try {
      const updatedProfile = await this.profileService.updateProfileFields(
        data.userId,
        data,
      );
  ...
  async subscribeProfileGrpc(data: SubscribeProfileRequest) {
  ...
  async createProfile(data: CreateUserProfileRequest) {
    console.log(data);
    try {
  ...
  async updateProfileSubscribe(data: UserProfileUpdateSubscribeRequest) {
    console.log(data);
    try {
      await this.profileService.updateProfilePayment(
        data.userId,
        data.paymentAccount,
      );
      return { status: 'ok' };
  ...
  async updateProfileAccount(
    data: { userId: string; paymentAccount: boolean }[],
  ) {
